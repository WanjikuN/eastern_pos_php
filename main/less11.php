...
<a href="expired.php">  <button class="btn btn-small" style="float: right;background:yellow; color: black"> Expired</button></a><br>
<a href="expsoon.php"> <button class="btn btn-small" style="float: right;background:green;color: blac"> Expiring soon</button></a><br>
<a href="less11.php"> <button class="btn btn-small" style="float: right;background:red;color: blac"> Quantity(less than 11) </button></a><br>
<a href="zero.php"> <button class="btn btn-small" style="float: right;background:black; color: white"> Quantity = 0</button></a><br>
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				include('../connect.php');
				$result = $db->prepare("SELECT *, price * qty as total FROM products WHERE qty <= 10 and qty!=0 ORDER BY product_id DESC");
				$result->execute();
			...
			<?php if((date('Y-m-d'))>=($row['expiry_date'])){
			?>
		<td style="background:yellow"><?php echo $row['expiry_date']; ?></td>
			<?php
			}elseif(((date('Y-m-d'))>=(date('Y-m-d',strtotime($row['expiry_date']. '- 6 days')))) AND (date('Y-m-d'))<($row['expiry_date']) ){
				?>
				<td style="background:green"><?php echo $row['expiry_date']; ?></td>
				<?php	
			...
			<?php if($row['qty']== 0) { ?>
			<td style="background:black;"><?php echo $row['qty']; ?></td>
			<?php }else { ?>
